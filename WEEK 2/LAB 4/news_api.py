...
class NewsAPI:
    ...
    def fetch_top_headlines(self, category="technology", country="us", max_articles=5):
        ...
        try:
            ...
            print(f"[OK] Fetched {len(processed)} articles from News API")
            return processed

        except requests.exceptions.RequestException as e:
            print(f"[ERR] Error fetching news: {e}")
            return []


    def fetch_ai_news(self, max_articles=5):
        """
        Search the /everything endpoint for AI-specific articles.

        Uses keyword search rather than a broad category so results are
        tightly focused on artificial intelligence topics.
        """
        self._wait_if_needed()

        params = {
            "apiKey":   self.api_key,
            "q":        "artificial intelligence OR machine learning OR LLM OR ChatGPT OR OpenAI OR Anthropic",
            "language": "en",
        ...
        try:
            ...
            print(f"[OK] Fetched {len(processed)} AI articles from News API")
            return processed

        except requests.exceptions.RequestException as e:
            print(f"[ERR] Error fetching AI news: {e}")
            return []
...
if __name__ == "__main__":
    ...
    print("--- Top headlines (technology) ---")
    ...
    print("\n--- AI keyword search ---")
    ai_articles = api.fetch_ai_news(max_articles=3)
